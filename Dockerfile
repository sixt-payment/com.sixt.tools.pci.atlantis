...
RUN apk add --no-cache ca-certificates gnupg curl git unzip bash openssh libcap openssl python3  && pip3 install boto3 &&  ln -s /usr/bin/python3 /usr/bin/python &&\
...
RUN AVAILABLE_TERRAFORM_VERSIONS="0.12.20" && \
